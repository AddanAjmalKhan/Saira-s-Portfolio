          ...
          return (
            <Reveal key={`${id ?? index}-${index}`} delay={index * 100} from="up" as="div">
              ...
              >
                ...
                <div className="mb-4 text-sm font-medium text-slate-200 flex items-center gap-2.5">
                  {item.logoUrl ? (
                    <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white p-1">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={item.logoUrl} alt={`${item.organisation || item.university} logo`} className="h-full w-full object-contain" />
                    </span>
                  ) : (
