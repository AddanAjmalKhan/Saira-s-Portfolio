...
import SummerSchoolsList from "@/components/sections/SummerSchoolsList";
import ConferencesList from "@/components/sections/ConferencesList";
import { getTrainings, getPageContent, getSummerSchools, getConferences } from "@/lib/content";

export const dynamic = "force-dynamic";

export default async function TrainingsPage() {
  const [trainings, pages, summerSchools, conferences] = await Promise.all([
    getTrainings(),
    getPageContent(),
    getSummerSchools(),
    getConferences(),
  ]);
  const block = pages["trainings"];
  return (
    <main className="min-h-screen pt-32 pb-20 sm:pt-40 relative overflow-hidden">
      <Aurora />
      <div className="mx-auto max-w-5xl px-4 sm:px-6 relative z-10">
        <Reveal from="up">
          {(block?.eyebrow ?? "Global Experience") && (
          ...
          <h1 className="mb-16 font-display text-4xl font-bold tracking-tight text-white sm:text-5xl">
            {block?.heading ? block.heading : (<>International <span className="text-gradient">Trainings &amp; Courses</span></>)}
          </h1>
        </Reveal>
              ...
              <div className="card-modern group flex h-full flex-col justify-between p-8 hover:-translate-y-1">
                <div>
                  ...
                  {training.description && (
                    <p className="text-slate-400 text-base leading-relaxed mb-6">{training.description}</p>
                  )}
                  <ImageRow images={training.images} className="mb-6" />
                </div>
        ...
        <SummerSchoolsList items={summerSchools} />
        <ConferencesList items={conferences} />
      </div>
