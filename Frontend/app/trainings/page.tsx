...
export default async function TrainingsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f6f8fb] pt-32 pb-20 sm:pt-40 relative overflow-hidden">
      <Aurora />
          ...
          {trainings.map((training, index) => (
            <Reveal key={index} delay={index * 100} from="up" as="div" className="h-full">
              <div className="card-modern group flex h-full flex-col justify-between p-8 hover:-translate-y-1">
                <div>
                  <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-mint/10 px-4 py-1.5 text-xs font-semibold text-mint transition-colors group-hover:bg-brand-grad group-hover:text-white">
                  ...
                  {training.description && (
                    <p className="text-slate-600 text-base leading-relaxed mb-8">{training.description}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 mt-auto pt-5 border-t border-slate-200 text-mint text-sm font-semibold">
