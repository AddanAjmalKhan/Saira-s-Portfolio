      ...
      <div className="mt-14 grid gap-6 md:grid-cols-2">
        {summerSchools.map((school, i) => (
          <Reveal key={school.title} from="up" delay={i * 100}>
            <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-gray-200 bg-gray-50 p-8 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_28px_60px_-30px_rgba(12,22,17,0.45)]">
              <span
                className="absolute right-5 top-4 font-display text-7xl font-semibold text-blue-500/10 transition-colors group-hover:text-blue-500/20"
                aria-hidden
              >
                0{i + 1}
              </span>
              ...
              <h3 className="mt-3 font-display text-2xl font-semibold leading-snug text-gray-900">
                {school.title}
              </h3>
              ...
              <p className="mt-1 inline-flex items-center gap-1.5 text-sm text-gray-600">
                <PinIcon width={14} height={14} className="text-blue-700" />
                {school.location}
              </p>
