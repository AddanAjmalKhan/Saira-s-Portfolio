...
export default function SummerSchools() {
  return (
    <Section id="summer-schools" className="bg-paper">
      <SectionHeading
        index="07"
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-2">
        {summerSchools.map((school, i) => (
          <Reveal key={school.title} from="up" delay={i * 100}>
            <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-ink/[0.08] bg-cream p-8 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_28px_60px_-30px_rgba(12,22,17,0.45)]">
              <span
              ...
              <span className="font-mono text-xs font-semibold uppercase tracking-widest text-amber-deep">
                {school.period}
              </span>
              ...
              <p className="mt-1 inline-flex items-center gap-1.5 text-sm text-ink-soft">
                <PinIcon width={14} height={14} className="text-amber-deep" />
                {school.location}
              </p>

              <ul className="mt-5 space-y-2 border-t border-ink/[0.07] pt-5">
