...
export default function About() {
  return (
    <section id="about" className="relative scroll-mt-24 bg-[#eef2f7] py-24 sm:py-28">
      <Container>
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          ...
          <Reveal from="left">
            <span className="inline-flex items-center gap-2 font-mono text-[0.7rem] font-semibold uppercase tracking-ultra text-mint">
              <Sparkle size={14} className="text-mint" /> About me
            </span>

            <h2 className="mt-5 font-display text-5xl font-semibold leading-[1.02] tracking-tightest text-slate-900 sm:text-6xl">
              Hello, <br />
            ...
            >
              ...
              <span className="ml-auto font-mono text-xs uppercase tracking-widest text-mint">
                Connect
              </span>
            </a>
            ...
            <div className="mt-8 flex flex-wrap gap-2.5">
              {focusAreas.map((area) => {
                ...
                return (
                  <span
                    key={area.title}
                    className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-mint/10 px-4 py-2 text-sm font-medium text-slate-800"
                  >
                    <Icon width={16} height={16} className="text-mint" />
                    {area.title}
                  </span>
                );
              })}
          ...
          <Reveal from="right" delay={120} className="flex flex-col gap-5">
            <div className="relative mx-auto w-full max-w-sm">
              <Sparkle
                size={26}
                className="absolute -right-2 -top-3 z-10 text-mint"
                animate
              />
              <Portrait
            ...
            {/* Contact card */}
            <div className="rounded-[1.75rem] bg-ink p-6 text-slate-900">
              <div className="flex items-center justify-between">
                <h3 className="font-display text-xl font-semibold">Contact</h3>
                <Sparkle size={18} className="text-mint" />
              </div>
              ...
              <a
                href="#contact"
                className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-ochre px-5 py-2.5 text-sm font-semibold text-slate-900 transition-colors hover:bg-ochre-deep"
              >
                Get in touch
